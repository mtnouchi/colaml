...
class StatsHandler:
    ...
    def compute(self, *, status='stdEM', return_logP=True):
        ...
        if not return_logP and self.logger is None: return
    
        logP = (self.stats.col_loglik.sum(), self.stats._model._log_prior_prob())
        if self.logger:
            self.logger.info(get_loggingformat(), EMrecord(*logP, self.stats._model.flat_params, status))
            
        if return_logP: return logP
...
class EpsRestartEM(Estimable):
    def __init__(self, stop_criteria=StopCriteria(atol=1e-8, rtol=1e-8), 
                 max_rounds=1000, restart_conf=EpsRestartConf(1, 0.1, 100)):
        self.stop_criteria = StopCriteria(*stop_criteria)
        self.max_rounds    = max_rounds
    ...
    def run(self, model, phytbl, show_progress=True, logger=None):
        with EMProgress(total=self.max_rounds, disable=not show_progress) as prog:
            ...
            while prog.count < self.max_rounds:
                ...
                if not (p_acc_new >= 0).all(): continue
                ...
                on_period = (prog.count - last_restart) % self.restart_conf.check_interval == 0
                if not (step_shrinks or on_period): continue
                ...
                if tmp_loglik + tmp_log_priorP < loglik + log_priorP: continue
...
class ParabolicEM(Estimable):
    def __init__(self, stop_criteria=StopCriteria(1e-8, 1e-8), max_rounds=1000, 
                 grid=BezierGeomGrid(0.1, 1.5), heuristics=False):
        self.stop_criteria = StopCriteria(*stop_criteria)
        self.max_rounds    = max_rounds
    ...
    def run(self, model, phytbl, show_progress=True, logger=None):
        with EMProgress(total=self.max_rounds, disable=not show_progress) as prog:
            ...
            while prog.count < self.max_rounds:
                ...
                if self.heuristics:
                    sequence = dropwhileN(
                        lambda p: not dist(p2, p) >= em_delta, sequence, 
                        max_drop=10 # to avoid infinite loop
                    )
                for _ in range(1000):
                    p_new = next(sequence)
                    if (p_new < 0).any(): break
                    ...
                    if tmp_log_jointP <= best_log_jointP: break
