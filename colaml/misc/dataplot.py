...
def draw_extant(phytbl, lmax=None, phyloorient='row', phylotreekw=None, **clustermapkw):
    ...
    default_cmap = ListedColormap([*map(plt.get_cmap('bone_r', lmax+1), range(1, lmax+1))])
    ...
    for config in dict(
        figsize=(20, 10), xticklabels=False, yticklabels=False, 
        cmap=default_cmap, cbar_pos=(0.02, 0.85, 0.03, 0.1), cbar_kws=dict(ticks=range(1, lmax+1))
    ).items():
        clustermapkw.setdefault(*config)
    
    if phyloorient == 'row':
        clustermapkw.setdefault('dendrogram_ratio', (0.2, 0.1))
        clst = sns.clustermap(OGs.T, mask=OGs.T.eq(0), col_cluster=True, row_cluster=False, vmin=0.5, vmax=lmax+0.5, **clustermapkw)
        phyplot.draw(phytbl.tree.to_ete3(), horizontal=True, loffs=0.5, lscale=1, **phylotreekw, ax=clst.ax_row_dendrogram)
        ...
        clst = sns.clustermap(OGs, mask=OGs.eq(0), col_cluster=False, row_cluster=True, vmin=0.5, vmax=lmax+0.5, **clustermapkw)
        phyplot.draw(phytbl.tree.to_ete3(), horizontal=False, loffs=0.5, lscale=1, **phylotreekw, ax=clst.ax_col_dendrogram)
...
def _iter_rect_statepatches(wsize, heights, drop_zero, wcenter=0, hoffs=0, cmap=None):
    ...
    for i, (h, top) in enumerate(zip(heights[start:], heights[start:][::-1].cumsum()[::-1]), start=start):
        yield Rectangle((-wsize/2+wcenter, top+hoffs), wsize, -h, fc=cmap(i), rotation_point=(0, 0))            

## PROVISIONAL
def draw_reconstruction(
    recon, major='states', minor=None, drop_zero_state=True, horizontal=False, style='bar', 
    nmajor=None, nminor=None, cmap_major=None, cmap_minor=None, size=(0.8, None), ax=None
):
    # resolve configs
    if major is None:
        raise ValueError('\'major\' should not be None.')
    
    config = dict(major=dict(n=nmajor, cmap=cmap_major), 
                  minor=dict(n=nminor, cmap=cmap_minor))
    ...
    data_maj, nmaj, cmap_maj, drop_zero_maj = map(config['major'].get, ('data', 'n', 'cmap', 'drop_zero'))
    data_mnr, nmnr, cmap_mnr, drop_zero_mnr = map(config['minor'].get, ('data', 'n', 'cmap', 'drop_zero'))
    
    # draw tree
    tree = recon.tree.to_ete3()
    plotter = phyplot.standardTreePlotter(node_text=None, show_confidence=False, horizontal=horizontal)
    ...
    wsize, hscale = size[0] * wscale, (size[1] or tree_height / 25) * hscale / recon.ncols
    ...
    for node in tree.traverse():
        ...
        generators.append(_iter_rect_statepatches(
        ...
        if minor is not None:
            ...
            for cntsub in cnt[smaj:,:]:
                ...
                generators.append(_iter_rect_statepatches(
                    wsize/4, hscale*cntsub, drop_zero_mnr, wsize*3/8, hscale*hoffs, cmap=cmap_mnr
                ))
    ...
    ax.add_collection(PatchCollection(
        patches, match_original=True, offsets=offsets, zorder=2, 
