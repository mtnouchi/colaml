...
class abstTreePlotter(metaclass=ABCMeta):
    def __init__(
        ...
        node_text='name',
        node_textkw=None,
        node_spacing='auto',  # global settings for nodes
        branch_text=None,
        branch_textkw=None,
        show_confidence=True,  # global settings for branches
    ):
        ...
        if branch_text is None:
            self.branch_text_fn = (
                (lambda node: None if node.is_leaf() else node.support)
                if show_confidence
                else (lambda _: None)
            )
    ...
    def _get_vertical_pos(self, tree):
        return {
            node: tree.get_distance(node, topology_only=not self.branch_length)
            + tree.dist
            for node in tree.traverse('postorder')
        }
    ...
    def draw(
        ...
        cladelw='auto',
        nodelw=None,
        local_node_textkw=None,
        local_branch_textkw=None,
        ax=None,
    ):
        if ax is None:
            ax = plt.gca()
        ...
        lateral_pos, vertical_pos = self._get_lateral_pos(tree), self._get_vertical_pos(
            tree
        )
        segments = self._make_segments(lateral_pos, vertical_pos)
        lc = self._resolve_tree_config(
            tree, 'color', self.globallc, clade=cladelc, node=nodelc
        )
        lw = self._resolve_tree_config(
            tree, 'width', self.globallw, clade=cladelw, node=nodelw
        )
        ...
        spacing = (
            max(vertical_pos.values()) / 100
            if self.node_spacing == 'auto'
            else self.node_spacing
        )
        for node, args, kwargs in self._iter_node_text_args(
            tree, lateral_pos, vertical_pos, spacing
        ):
        ...
        for node, args, kwargs in self._iter_branch_text_args(
            tree, lateral_pos, vertical_pos
        ):
        ...
        if self.align:
            align_segments = self._make_align_segments(
                lateral_pos, vertical_pos, spacing
            )
            ax.add_collection(
                LineCollection(align_segments.values(), **self.align_linekw)
            )
...
class standardTreePlotter(abstTreePlotter):
    ...
    def _make_segments(self, lateral_pos, vertical_pos):
        ...
        return {
            node: (
                [
                    (
                        self._ladjust(lateral_pos[node]),
                        self._vadjust(vertical_pos[node]),
                    )[::step],
                    (
                        self._ladjust(lateral_pos[node]),
                        self._vadjust(vertical_pos[node.up]),
                    )[::step],
                    (
                        self._ladjust(lateral_pos[node.up]),
                        self._vadjust(vertical_pos[node.up]),
                    )[::step],
                ]
                if not node.is_root()
    ...
    def _make_align_segments(self, lateral_pos, vertical_pos, spacing):
        ...
        return {
            node: [
                (self._ladjust(lateral_pos[node]), self._vadjust(vertical_pos[node]))[
                    ::step
                ],
                (
                    self._ladjust(lateral_pos[node]),
                    self._vadjust(tree_height + spacing),
                )[::step],
            ]
            for node in lateral_pos.keys() & vertical_pos.keys()
            if node.is_leaf()
    ...
    def _make_segments(self, lateral_pos, vertical_pos):
        ...
        return {
            node: [
                ...
                (
                    self._ladjust(lateral_pos.get(node.up, lateral_pos[node])),
                    self._vadjust(vertical_pos.get(node.up, 0)),
                )[::step],
            ]
            for node in lateral_pos.keys() & vertical_pos.keys()
            # if not node.is_root()
...
class circularTreePlotter(abstTreePlotter):
    ...
    def _make_segments(self, lateral_pos, vertical_pos):
        ...
        for node in lateral_pos.keys() & vertical_pos.keys():
            r0, rad0 = self._radjust(vertical_pos[node]), self._aadjust(
                lateral_pos[node]
            )
            r1, rad1 = self._radjust(vertical_pos.get(node.up, 0)), self._aadjust(
                lateral_pos.get(node.up, lateral_pos[node])
            )
    ...
    def _iter_node_text_args(self, tree, lateral_pos, vertical_pos, spacing):
        ...
        for node in tree.traverse('preorder'):
            ...
            if text is None:
                continue
            yield node, (r * np.cos(rad), r * np.sin(rad), text), dict(
...
def draw(
    ...
    **general_kw
    ...
    return plotter.draw(
        ...
        cladelw=cladelw,
        nodelw=nodelw,
        local_node_textkw=local_node_textkw,
        local_branch_textkw=local_branch_textkw,
    )
