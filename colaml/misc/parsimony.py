...
def min_changes(phytbl):
    ...
    for current, partial_cols in zip(phytbl.tree.nodes, phytbl.inside_encoding):
        if current.is_leaf():
            ...
            tmp = np.sum([
                (minchanges[ch][:,newax,uniq] + diff[:,:,newax]).min(axis=0) # an,ab->bn
...
def mean_mp_changes(phytbl):
    ...
    min_changes      = [np.zeros((nst, nuniqcols_m))
                        for nuniqcols_m in nuniqcols]               # (m, a, n)
    aux_min_changes  = [np.zeros((nst, nuniqcols_m))
                        for nuniqcols_m in nuniqcols]               # (m, a, n)
    inside_ways      = [np.empty((nst, nuniqcols_m), dtype=object)
                        for nuniqcols_m in nuniqcols]               # (m, a, n)
    aux_inside_ways  = [np.empty((nst, nuniqcols_m), dtype=object)
    ...
    argmin_changes   = [np.empty((nst, nst, nuniqcols_m), dtype=bool)
                        for nuniqcols_m in nuniqcols] 
    ...
    for current_idx in range(nnodes):
        ...
        tmp_changes = min_changes[current_idx][:,newax,:] + diff[:,:,newax] # (a, b, n)
        aux_min_changes[current_idx][:,:] = tmp_changes.min(axis=0)
        argmin_changes [current_idx][:,:,:] = (tmp_changes == aux_min_changes[current_idx])
        aux_inside_ways[current_idx][:,:] = np.where(
            argmin_changes[current_idx], inside_ways[current_idx][:,newax,:], 0
        ).sum(axis=0)
        
    outside_ways[-1,:,:] = (min_changes[-1] == min_changes[-1].min(axis=0))[:,phytbl.inside_encoding[-1].codes]
    total_ways = (inside_ways[-1][:,phytbl.inside_encoding[-1].codes] * outside_ways[-1]).sum(axis=0)
    for current_idx in reversed(range(nnodes - 1)):
        ...
        aux_outside_ways[current_idx][:,:] = (
             ...
             * (inside_ways[parent_idx]
                 // aux_inside_ways[current_idx][:,cur2par_codes])[:,parent_pcols.codes]
        ) # bn,(bn~,bn~~)->bn
        
        outside_ways[current_idx,:,:] = np.where(
            argmin_changes[current_idx][:,:,current_pcols.codes], aux_outside_ways[current_idx][newax,:,:], 0
        ).sum(axis=1)
    ...
    for current_idx in range(nnodes - 1):
        ...
        trans_ways = (
            (inside_ways[current_idx][:,newax,:] * argmin_changes[current_idx])[:,:,current_pcols.codes] * 
            aux_outside_ways[current_idx][newax,:,:]
        )
        assert np.alltrue(trans_ways.sum(axis=(0,1)) == total_ways)
        mean_mp_changes.append((trans_ways * diff[:,:,newax]).sum(axis=(0,1)) / total_ways)
