...
def plot_substmodel(model, state_labels=None, ax=None, **heatmapkw):
    if isinstance(model, BDARD):
        ax = _plot_standard_substmodel(model, state_labels=state_labels, ax=ax, **heatmapkw)
    elif isinstance(model, MarkovModulatedBDARD):
        ax = _plot_Markov_modulated_substmodel(model, state_labels=state_labels, ax=ax, **heatmapkw)
    elif isinstance(model, tuple):
        ax = _plot_Mirage_substmodels(model, state_labels=state_labels, ax=ax, **heatmapkw)
    else:
...
def _plot_standard_substmodel(model, state_labels=None, ax=None, **heatmapkw):
    ...
    heatmapkw.setdefault('norm', colors.LogNorm(vmin=R[~mask].min(), vmax=R[~mask].max()))
...
def _plot_Markov_modulated_substmodel(model, state_labels=None, ax=None, **heatmapkw):
    ...
    # plot heatmap
    ax = _plot_standard_substmodel(model, state_labels=None, ax=ax, **heatmapkw) # NB: state_labels=None is intentional
    ...
    for k in range(ncat):
        ...
        ysep.extend([[(0, k*nchrst+i), (1, k*nchrst+i)] for i in range(1, nchrst)])
        xsep.extend([[(k*nchrst+i, 0), (k*nchrst+i, 1)] for i in range(1, nchrst)])
        ...
        ax.add_collection(PatchCollection(diagfr, ec='k', fc='none', lw=plt.rcParams['lines.linewidth'] * 1.5, clip_on=False))
    ...
    for k in range(1, ncat):
        catsep.extend([[(nchrst*k, 0), (nchrst*k, nchrst*ncat)],
