...
class BranchwiseTreeModel(AbstTreeModel):
    ...
    def __init__(self, nbranches, ncharstates, substmodelclass, prior_kw=None, init_params_method=None, init_params_kw=None):
    ...
    def init_params_emEM(self, phytbl, rng, ntrial=10, max_rounds=100, stop_criteria=StopCriteria(1e-4, 1e-4), show_init_progress=False, distr_kw=None, **fit_kw):
        ...
        super().init_params_emEM(
            phytbl=phytbl, rng=rng, ntrial=ntrial, show_init_progress=show_init_progress, distr_kw=distr_kw, max_rounds=max_rounds, stop_criteria=stop_criteria, **fit_kw
        )
    ...
    def _decompress_flat_params(self, p):
        ...
        return dict(
            ...
            substmodels_params=[
                ...
                for sbstmdl, p_sbstmdl in zip(self.__substmodels, p[nchrst:].reshape(self.nbranches,-1))
            ]
        )
    ...
    def _log_prior_prob(self):
        ...
        return logP_root_prior + sum(sbstmdl._log_prior_prob() for sbstmdl in self.__substmodels)
    
    
    def fit_EM(self, phylodata, stop_atol=1e-8, stop_rtol=1e-8, max_rounds=100, show_progress=False, logger=None):
        return super().fit_EM(
            phylodata=phylodata, 
            stop_atol=stop_atol, 
            stop_rtol=stop_rtol, 
    ...
    def fit_epsR_EM(self, phylodata, stop_atol=1e-8, stop_rtol=1e-8, max_rounds=1000, show_progress=False, logger=None, restart_params=(1, 0.1), restart_check_intv=100):
        super().fit_epsR_EM(
            phylodata=phylodata, 
            stop_atol=stop_atol, 
            stop_rtol=stop_rtol, 
    ...
    def fit_parabolic_EM(self, phylodata, stop_atol=1e-8, stop_rtol=1e-8, max_rounds=1000, show_progress=False, logger=None, grid_params=(0.1, 1.5), heuristics=False):
    ...
    def _reconstruct_marginal(self, phytbl):
        ...
        insideP = np.array([
            ...
            for insideP_m, partial_cols in zip(stats.insideP, phytbl.inside_encoding)
        ])
    ...
    def _wrap_reconstruction(self, phytbl, recon_charstates, method=None):
        ...
        return ReconPhyTable(
            dict(zip(tree2.names, recon_charstates.T)), tree2, 
            metadata_kw=dict(method=method, **self._get_model_info())
        )
    
    def reconstruct_joint(self, phylodata):
        return self._wrap_reconstruction(phylodata, self._reconstruct_joint(phylodata), 'joint')
        
    def reconstruct_marginal(self, phylodata):
        return self._wrap_reconstruction(phylodata, self._reconstruct_marginal(phylodata), 'marginal')
