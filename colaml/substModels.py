...
class eigSuffStatsEngine(abstSuffStatsEngine):
        ...
        for u in range(nst):
            for v in range(u):
                ...
                if np.abs(den) < 1e-8:
                    ...
                    integ[u, v] = integ[v, u] = np.exp((tD[v] + tD[u]) / 2) * (
                        1 + den**2 / 24
                    )
                else:
    ...
    def compute(self, substmodel, t):
        ...
        tPdP = (UinvU1 @ integ @ UinvU2).reshape(nst, nst, nst, nst).swapaxes(
            1, 2
        ).real * t
...
class auxRSuffStatsEngine(abstSuffStatsEngine):
    ...
    def precompute(self, substmodel):
        ...
        auxRs = np.array(
            [
                ...
                for I in np.eye(ncmpst * ncmpst).reshape(
                    (ncmpst, ncmpst, ncmpst, ncmpst)
                )[substmodel._mask_nonzero]
            ]
        )
        return self.precompResult(
            auxRs=auxRs, zipped_coef=coef[substmodel._mask_nonzero]
        )
    ...
    def compute(self, substmodel, t):
        ...
        assert not (
            tPdP < 0
        ).any(), f'tPdP < 0 in {[*zip(*np.where(tPdP < 0))]}: {tPdP[tPdP < 0]}'
        stats = np.zeros((ncmpst, ncmpst, ncmpst, ncmpst))
        stats[:, :, substmodel.mask_nonzero] = c_einsum(
            'hab,h->abh', tPdP, cache.zipped_coef
        )
        return stats
...
class spsSuffStatsEngine(abstSuffStatsEngine):
    ...
    def compute(self, substmodel, t):
        ...
        tR = t * substmodel.R
        tPdP = np.array(
            [expm_frechet(tR, tE, compute_expm=False) for tE in t * cache.Es]
        )
        assert not (
            tPdP < 0
        ).any(), f'tPdP < 0 in {[*zip(*np.where(tPdP < 0))]}: {tPdP[tPdP < 0]}'
...
class abstSubstModel(ABC):
    _available_engine = MappingProxyType(
        dict(eig=eigSuffStatsEngine, auxR=auxRSuffStatsEngine, sps=spsSuffStatsEngine)
    )
    ...
    def init_params(self, init_params_method, init_params_kw):
        if init_params_method is None:
            warnings.warn(
                'Parameters are not initialized. Call either \'init_params_manual\', \'init_params_random\' before calculating statistics.'
            )
        elif init_params_method == 'skip':
            if init_params_kw is not None:
                warnings.warn(
                    'When init_params_method=\'skip\', init_params_kw is ignored.'
                )
...
class BDARD(abstSubstModel):
    ...
    def __init__(
        ...
        # init masks
        self._mask_ns = np.eye(ncharstates, k=1, dtype=bool) | np.eye(
            ncharstates, k=-1, dtype=bool
        )
    ...
    def _get_next_params(self, tfdns):
        ...
        R = (tfdns + rates_gamma.shape - 1) / (
            np.diagonal(tfdns) + 1 / rates_gamma.scale
        )[newax, :]
    ...
    def _log_prior_prob(self):
        rates_gamma = self.__rates_prior
        logP_rates_prior = (
            xlogy(rates_gamma.shape - 1, self.flat_params).sum()
            - self.flat_params.sum() / rates_gamma.scale
        )
...
class MarkovModulatedBDARD(abstSubstModel):
    ...
    def __init__(
        ...
        # init masks
        self._mask_cpy_ns = np.kron(
            np.eye(ncategories, dtype=bool),
            np.eye(ncharstates, k=1, dtype=bool)
            | np.eye(ncharstates, k=-1, dtype=bool),
        )
        self._mask_switch = np.kron(
            np.ones((ncategories, ncategories), dtype=bool),
            np.eye(ncharstates, dtype=bool),
        )
        self._cat_idx = tuple(
            np.hstack(
        ...
        cpyidx_row = (
            (icat * ncharstates)[:, None] + np.hstack((icpy - 1, icpy))
        ).reshape(-1)
        cpyidx_col = (
            (icat * ncharstates)[:, None] + np.hstack((icpy, icpy - 1))
        ).reshape(-1)
        self._flatidx = (
            np.hstack((catidx_row, cpyidx_row)),
            np.hstack((catidx_col, cpyidx_col)),
        )

        # set priors
        assume_priors = prior_kw is not None
        if assume_priors:
            cpy_changeR_prior, cat_switchR_prior = self._params_signature.bind(
                **prior_kw
            ).args
        ...
        self.update(
            cpy_change_rates=cpy_change_rates, cat_switch_rates=cat_switch_rates
        )
    ...
    def init_params_manual(self, cpy_change_rates, cat_switch_rates):
        ...
        cat_switch_rates = _to_ndarray_of_shape(
            cat_switch_rates, (2, ncat * (ncat - 1) // 2)
        )
    ...
    def init_params_random(self, rng, distr_kw=None):
        ...
        if self.__assume_priors:
            ...
            cpy_changeR_distr, cat_switchR_distr = self._params_signature.bind(
                **distr_kw
            ).args
        ...
        self.update(
            cpy_change_rates=(
                ...
                if self.__has_invariant_cat
                else rng.gamma(*cpy_changeR_distr, size=(ncat, 2, nchrst - 1))
            ),
            cat_switch_rates=rng.gamma(
                *cat_switchR_distr, size=(2, ncat * (ncat - 1) // 2)
            ),
        )
    ...
    def _get_next_params(self, tfdns):
        ...
        cpy_change_rates = (
            (
                (tfdns + cpy_change_gamma.shape - 1)
                / (np.diag(tfdns)[newax, :] + 1 / cpy_change_gamma.scale)
            )[self._mask_cpy_ns]
            .reshape((ncat, nchrst - 1, 2))
            .swapaxes(1, 2)
        )
        ...
        tmp2 = (tfdns_cat + cat_switch_gamma.shape - 1) / (
            np.diag(tfdns_cat)[newax, :] + 1 / cat_switch_gamma.scale
        )
        cat_switch_rates = tmp2[self._cat_idx].reshape(2, -1)
        ...
        return dict(
            cat_switch_rates=cat_switch_rates, cpy_change_rates=cpy_change_rates
        )
    ...
    def _log_prior_prob(self):
        ...
        p = self.flat_params
        cpy_change_rates = p[
            ncat * (ncat - 1) + int(self.has_invariant_cat) * (2 * nchrst - 2) :
        ]
        # cat_switch_rates = p[0] if share_switch_rate else p[:ncat*(ncat-1)]
        cat_switch_rates = p[: ncat * (ncat - 1)]

        logP_cpy_changeR_prior = (
            xlogy(cpy_change_gamma.shape - 1, cpy_change_rates).sum()
            - cpy_change_rates.sum() / cpy_change_gamma.scale
        )
        logP_cat_switchR_prior = (
            xlogy(cat_switch_gamma.shape - 1, cat_switch_rates).sum()
            - cat_switch_rates.sum() / cat_switch_gamma.scale
        )
