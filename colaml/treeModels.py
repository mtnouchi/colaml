from .phyTables import ReconPhyTable, PostorderSerializedTree
from .treeStats import SingleRateTreeStats, MultipleRateTreeStats
from .fitting import PlainEM, EpsRestartEM, ParabolicEM
...
class AbstTreeModel(ABC):
    @describe_estimator
    def fit(self, phytbl, *, 
            method='EM', show_progress=True, logger=None, **fit_kw):
        """
    ...
    def init_params(self, init_params_method, init_params_kw):        
        if init_params_method is None:
            warnings.warn('Parameters are not initialized. Call either \'init_params_manual\', \'init_params_random\' or \'init_params_emEM\' before calculating statistics or fitting parameters.')
    ...
    def init_params_emEM(self, phytbl, rng, ntrial, show_init_progress, distr_kw, **fit_kw):
...
class PlainTreeModel(AbstTreeModel):
    ...
    def __init__(self, ncharstates, substmodelclass, prior_kw=None, init_params_method=None, init_params_kw=None):
    ...
    def init_params_random(self, rng, distr_kw=None):
        if self.__assume_priors:
            if distr_kw is not None:
                 warnings.warn('distr_kw is ignored because this model already assumes priors.')
            ...
            if rootP_distr is None:
                ...
                rootP_distr = Dirichlet(_to_ndarray_of_shape(rootP_distr, (self.ncharstates,)))
        
        self.substmodel.init_params_random(rng, distr_kw)
        self._update(root_probs=rng.dirichlet(rootP_distr))
        
    def init_params_emEM(self, phytbl, rng, ntrial=10, max_rounds=100, stop_criteria=(1e-4, 1e-4), show_init_progress=False, distr_kw=None, **fit_kw):
        ...
        AbstTreeModel.init_params_emEM(
            self, phytbl=phytbl, rng=rng, ntrial=ntrial, show_init_progress=show_init_progress, distr_kw=distr_kw, max_rounds=max_rounds, stop_criteria=stop_criteria, **fit_kw
        )
    ...
    def _get_next_params(self, stats):
        ...
        return dict(root_probs=post_rootP, **self.substmodel._get_next_params(stats.tfdns))
    ...
    def _reconstruct_joint(self, phytbl):
        ...
        for current_idx in range(nnodes - 1):
            ...
            if current.is_leaf():
                ...
                tmp = log_branchP[current_idx,:,:,newax] + np.sum([
                    loglik[child_idx,:,:] 
        ...
        recon_charstates[:,-1] = (log_rootP[:,newax] + np.sum([
            loglik[child_idx,:,:]
            for child_idx in tree.nodes[-1].children_idxs
        ], axis=0)).argmax(axis=0)
        
        for current_idx in reversed(range(nnodes - 1)):
            parent_idx = tree.nodes[current_idx].parent_idx
            recon_charstates[:,current_idx] = tmp_recon[current_idx,recon_charstates[:,parent_idx],np.arange(ncols)]
    ...
    def _wrap_reconstruction(self, phytbl, recon_charstates, method=None):
        ...
        return ReconPhyTable(
            dict(zip(tree.names, recon_charstates.T)), tree, 
            metadata_kw=dict(method=method, **self._get_model_info())
        )
    
    def reconstruct_joint(self, phytbl):
        return self._wrap_reconstruction(phytbl, self._reconstruct_joint(phytbl), 'joint')
        
    def reconstruct_marginal(self, phytbl):
        return self._wrap_reconstruction(phytbl, self._reconstruct_marginal(phytbl), 'marginal')
    ...
    def _simulate(self, tree, N, rng): 
        ...
        # init
        root = next(preorder)
        charstates[root.name] = rng.choice(*self.root_probs.shape, size=N, p=self.root_probs)
        ...
        for node in preorder:
            ...
            for st in set(pa_charstates):
                ...
                my_charstates[mask] = rng.choice(*P[:,st].shape, size=np.count_nonzero(mask), p=P[:,st])
...
class MixtureTreeModel(AbstTreeModel):
    _params_signature = signature(lambda mixture_probs, root_probs, **substmodels_params: ...)
    
    def __init__(self, ncharstates, nmixtures, substmodelclass, prior_kw=None, init_params_method=None, init_params_kw=None):
        ...
        if assume_priors:
            ...
            mixtureP_prior = Dirichlet(_to_ndarray_of_shape(mixtureP_prior, (nmixtures  , )))
            rootP_prior    = Dirichlet(_to_ndarray_of_shape(rootP_prior   , (ncharstates, )))
        else:
    ...
    def init_params_manual(self, mixture_probs, root_probs, substmodels_params):
        ...
        self.update(mixture_probs=mixture_probs, root_probs=root_probs, substmodels_params=None)
    ...
    def _decompress_flat_params(self, p):
        ...
        return dict(
            ...
            substmodels_params=[
                ...
                for sbstmdl, p_sub in zip(self.__substmodels, p[nmixt*(nchrst+1):].reshape((nmixt, -1)))
            ]
        ) 
        
    def _get_next_params(self, stats):
        post_mixtureP  = stats.post_mixtureP.sum(axis=1) + self.__mixtureP_prior - 1 # (kn->k) + k
        post_mixtureP /= post_mixtureP.sum()
        
        post_rootP  = (stats.post_mixtureP[:,newax,:] * stats.post_rootP).sum(axis=2) + (self.__rootP_prior - 1)[newax,:]# (kn,kan->ka) + a
        ...
        return dict(mixture_probs=post_mixtureP, root_probs=post_rootP, substmodels_params=substmodels_params)
    ...
    def _log_prior_prob(self):
        ...
        return logP_mixture_prior + logP_root_prior + sum(sbstmdl._log_prior_prob() for sbstmdl in self.__substmodels)
    ...
    def _reconstruct_joint(self, phytbl):
        ...
        tmp_recon = np.zeros((nnodes-1, nmixt, ncmpst, ncols), dtype=int) # (m, k, j, n)
        ...
        tmp = log_rootP[:,:,newax] + np.sum([
            loglik[child_id,:,:,:] 
            for child_id in tree.nodes[-1].children_idxs
        ], axis=0) # (k, j, n)

        recon_mixtures, recon_charstates[:,-1] = np.column_stack(
            np.unravel_index(
                tmp.reshape(-1, tmp.shape[-1]).argmax(axis=0), 
                tmp.shape[:-1]
            )
        ).T # argmax_kj(k, j, n) -> (n,)
        ...
        for current_idx in reversed(range(nnodes - 1)):
            parent_idx = tree.nodes[current_idx].parent_idx
            recon_charstates[:,current_idx] = tmp_recon[current_idx,recon_mixtures,recon_charstates[:,parent_idx],np.arange(ncols)]
    ...
    def _reconstruct_marginal(self, phytbl):
        ...
        outsideP = stats.outsideP[:,recon_mixtures,:,np.arange(phytbl.ncols)] # (m, k, a, n) -> (n, m, a); the order of axes has been changed by fancy indexing
    ...
    def _wrap_reconstruction(self, phytbl, recon_charstates, recon_mixtures, method=None):
        tree = deepcopy(phytbl.tree)
        tree.fill_names(ignore_tips=True)
        return ReconPhyTable(
            dict(zip(tree.names, recon_charstates.T)), tree, 
    ...
    def simulate(self, tree, N, rng):
        ...
        for mix in set(mixture_ids):
            ...
            my_charstates[mask] = rng.choice(*rootP.shape, size=np.count_nonzero(mask), p=rootP)
        ...
        for node in preorder:
            ...
            for mix, st in set(zip(mixture_ids, pa_charstates)):
                ...
                my_charstates[mask] = rng.choice(*P[mix,:,st].shape, size=np.count_nonzero(mask), p=P[mix,:,st])
            charstates[node.name] = my_charstates
...
class MarkovModulatedTreeModel(PlainTreeModel):
    _params_signature = signature(lambda cpy_root_probs, cat_root_probs, **substmodel_params: ...)
    
    def __init__(self, ncharstates, ncategories, substmodelclass, invariant_cat=False, prior_kw=None, init_params_method=None, init_params_kw=None):
        # inspect prior_kw and set priors
        assume_priors = prior_kw is not None
        if assume_priors:
            bound_args = self._params_signature.bind(**prior_kw)
            ...
            cpy_rootP_prior = Dirichlet(_to_ndarray_of_shape(cpy_rootP_prior, (ncharstates, )))
            cat_rootP_prior = Dirichlet(_to_ndarray_of_shape(cat_rootP_prior, (ncategories, )))
        else:
        ...
        # define parameters (and init them with NaN)
        self.__substmodel = substmodelclass(
            ncharstates, ncategories, invariant_cat=invariant_cat, 
            prior_kw=prior_kw, init_params_method='skip', 
        )
    ...
    def init_params_random(self, rng, distr_kw=None):
        if self.__assume_priors:
            ...
            if cpy_rootP_distr is None:
                ...
                cpy_rootP_distr = _to_ndarray_of_shape(cpy_rootP_distr, (self.ncharstates,))
            if cat_rootP_distr is None:
                cat_rootP_distr = np.ones(self.ncategories)
            else:
                cat_rootP_distr = _to_ndarray_of_shape(cat_rootP_distr, (self.ncategories,))
    ...
    def init_params_emEM(self, phytbl, rng, ntrial=10, max_rounds=100, stop_criteria=(1e-4, 1e-4), show_init_progress=False, distr_kw=None, **fit_kw):
        if not self.__assume_priors:
            ...
            distr_kw.setdefault('cat_switch_rates', (1.0, mp_rate * 0.1)) # sparse modeling (to be reconsidered)
    ...
    def update(self, cpy_root_probs=None, cat_root_probs=None, **kwargs):
        if cpy_root_probs is None:
            ...
            cpy_root_probs = _to_ndarray_of_shape(cpy_root_probs, self.__cpy_rootP.shape)
              
        if cat_root_probs is None:
            cat_root_probs = self.__cat_rootP
        else:
            cat_root_probs = _to_ndarray_of_shape(cat_root_probs, self.__cat_rootP.shape)
    ...
    def _get_next_params(self, stats):
        ...
        return dict(
            cat_root_probs=cat_rootP, cpy_root_probs=cpy_rootP, 
            **self.substmodel._get_next_params(stats.tfdns)
        )

    def _log_prior_prob(self):
        logP_cpy_root_prior = xlogy(self.__cpy_rootP_prior - 1, self.cpy_root_probs).sum()
        logP_cat_root_prior = xlogy(self.__cat_rootP_prior - 1, self.cat_root_probs).sum()
        
        return logP_cpy_root_prior + logP_cat_root_prior + self.__substmodel._log_prior_prob()
        
    def _reconstruct_joint(self, phytbl):
        recon_compound_states = super()._reconstruct_joint(phytbl)
        recon_categories, recon_charstates = np.divmod(recon_compound_states, self.ncharstates)
    ...
    def _wrap_reconstruction(self, phytbl, recon_charstates, recon_categories, method):
        ...
        return ReconPhyTable(
            dict(zip(tree.names, recon_charstates.T)), tree, 
            otherstates = (
                ('categories', dict(zip(tree.names, recon_categories.T))),
            ),
            metadata_kw=dict(method=method, **self._get_model_info())
        )
    ...
    def simulate(self, tree, N, rng):
        ...
        charstates = {node: states %  self.ncharstates for node, states in compound_states.items()}
        categories = {node: states // self.ncharstates for node, states in compound_states.items()}
