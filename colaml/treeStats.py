...
class SingleRateTreeStats:
    ...
    def compute(self):
        ...
        assert (
            self.tfdns[sbstmdl._mask_nonzero] >= 0
        ).all(), f'tfdns < 0 in {[*zip(*np.where(self.tfdns < 0))]}: {self.tfdns[self.tfdns < 0]}'
...
class MultipleRateTreeStats:
    ...
    def __init__(self, model, phytbl):
        ...
        self.aux_insideP = [
            np.empty((nmixt, ncmpst, nuniqcols_m)) for nuniqcols_m in nuniqcols
        ]  # (m, k, b, n)
        self.insideP = [
            np.empty((nmixt, ncmpst, nuniqcols_m)) for nuniqcols_m in nuniqcols
        ]  # (m, k, a, n)
        self.scale = [
            np.empty((nmixt, nuniqcols_m)) for nuniqcols_m in nuniqcols
        ]  # (m, k, n)
    ...
    def compute(self):
        ...
        for current_idx in range(nnodes):
            ...
            if current_idx == nnodes - 1:
                break
            np.matmul(
                self.branchP[current_idx].swapaxes(1, 2),
                self.insideP[current_idx],
                out=self.aux_insideP[current_idx],
            )  # kan,kab->kbn
        ...
        self.post_rootP = c_einsum('kan,ka->kan', self.insideP[-1], model.root_probs)[
            :, :, phytbl.inside_encoding[-1].codes
        ]  # kan,ka->kan
        ...
        for current_idx in range(nnodes - 1):
            ...
            self.tfdns[mask] += c_einsum(
                ...
                np.matmul(  # kan,kbn,kn->kab
                    self.insideP[current_idx][:, :, partial_cols.codes],
                    (
                        self.aux_outsideP[current_idx] * self.post_mixtureP[:, newax, :]
                    ).swapaxes(1, 2),
                )[rept, :, :],
            )
        ...
        assert (
            self.tfdns >= 0
        ).all(), f'tdfns < 0 in {[*zip(*np.where(self.tfdns < 0))]}: {self.tfdns[self.tfdns < 0]}'
